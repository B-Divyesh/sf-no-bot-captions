...
const legalPages: Record<string, { title: string; updated: string; body: string }> = {
  '/privacy': {
    ...
    body: `
      ...
      <section><h2>Data our server receives</h2><p>We keep a privacy-respecting daily page count containing only the page path, date, and total. It has no cookie, account, fingerprint, IP address, or meeting data. To protect that count endpoint, a temporary in-memory rate limit may use a network address for up to one minute. It is never written to the count. Standard short-lived infrastructure logs may contain request metadata for security and reliability.</p></section>
